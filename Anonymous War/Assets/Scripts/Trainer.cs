...
public class Trainer : PlayerController
{
    ...
    void AIGuide1()
    {
        ...
        {
            TrainerNextStep = 7;
            Root.instance.flowchart.SetIntegerVariable("GuideStep",-6);
            TrainnerMove(BoardManager.Grounds[5][4]);
        }
        if(Step==7)
        {
            CheckAttack();
        ...
        if(Step==12)
        {
            ...
            TrainnerMove(BoardManager.Grounds[4][4]);
        }
        if(Step==13)
        {
            CheckAttack();
    ...
    void TrainnerMove(GameObject GroundToMove)
    {
        ...
        Root.instance.flowchart.SetIntegerVariable("GuideStep", TrainerNextStep);
    }
    void TrainnerAttack(GameObject PlayerToAttack)
    {
        //对接攻击函数，可以不用看了
        //获取反击攻击力，反击范围与双方血条
    ...
    public static IEnumerator TrainerWait()
    {
    ...
    void AIMove()
    {
        ...
        //Difference:act when enemy blood sum 5 less than that of player
        if (PlayerBloodSum < EnemyBloodSum + 6)
        {
            Debug.Log("Act");
            for (int i = 0; i < GameManager.OccupiedGround.Count; i++)
            {
        ...
        if(PlayerBloodSum >= EnemyBloodSum + 6||(GroundToMove==null&&possibleEnemy==null))
        {
            Debug.Log("Normal");
            for (int i = 0; i < GameManager.OccupiedGround.Count; i++)
            {
